...
def get_repeats(text, r):
	"""
	Функція отримання значення D для відповідного значення r
	"""
	D = 0
...
def get_blocks(text, r):
	"""
	Функція розбиття тексту на блоки відповідно до значення r
	"""
	blocks = []
	for i in range(r):
		s = ''
		for j in range(0, len(text)-i, r):
			s += text[j+i]
		blocks.append(s)
...
def decrypt(text: str, key: str, ab: str):
	...
	for i in range(len(text)):
		c = ab.index(text[i])
		k = ab.index(key[i%mod])
		m = ab[c-k]
		result += ab[c-k]

	return result

def encrypt(text, key, ab):
	"""
